 ...
 * getCarInfoById returns a string in the format `This is a {car_make} {car_model}d
 * 
 * For example, if getCarInfoById is invoked with the inventory and the number 1,
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, id) {
  const getCarInfo = inventory.find((item, id) => {
    return id === 0;
  })
...
function sortCarInventory(inventory) {
  const sortCarModel = inventory.sort(function(a, b){
    var carModel1 = a.car_model;
    var carModel2 = b.car_model;
    if (carModel1 < carModel2) {
      return -1;
    }
    if (carModel2 > carModel1) {
      return 1;
    }
  });
  return sortCarModel;
...
function getModelYears(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year === inventory.car_year) {
      modelYears.push(inventory[i]);
    }
  }
  return modelYears;
}
